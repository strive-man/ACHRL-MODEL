...
import scipy.sparse as sp # scipy.sparse 矩阵压缩的库函数
import numpy as np
...
class Dataset(object):
    ...
    Loading the data file
        ...
        trainMatrix：加载 评级记录 作为类数据的 稀疏矩阵
        trianList: load rating records as list to speed up user's feature retrieval
        trianList：加载 评级记录 作为列表 以加速用户的特征检索
        testRatings: load leave-one-out rating test for class Evaluate leave-one-out：一种交叉验证方法
        testRatings：为类 Evaluate 加载 “leave-one-out”评级测试
        testNegatives: sample the items not rated by user
        testNegatives：采样未被用户评分的项目
    '''

    def __init__(self, data_path, num_negatives, batch_size, fast_running=False):
        '''
        Constructor ：构造函数 ，num_negatives：负例
        rating：评级，等级 用户对item的偏好的表达。评分可以是二分类的（如喜欢和不喜欢），
        也可以是整数（如1到5星）或连续（某个间隔的任何值）。
        另外，还有一些隐反馈，只记录一个用户是否与一个项目进行了交互。
        '''
        ...
        assert len(self.testRatings) == len(self.testNegatives) #这个断言应该就是理解为，保证二者的长度是一样长的
    ...
    def load_training_file_as_matrix(self, filename):
        ...
        with open(filename, "r") as f:
            ...
            while line != None and line != "":
                ...
                line = f.readline()
        # Construct matrix  dok_matrix 是系统函数，导入的包内的函数
        ...
        with open(filename, "r") as f:
            ...
            while line != None and line != "":
                ...
                if (rating > 0):
                    mat[user, item] = 1.0
                line = f.readline()
        print("already load the trainMatrix...")# 在这个位置，把这个训练矩阵加载完毕了
        return mat
    ...
    def load_training_file_as_list(self, filename):
        ...
        with open(filename, "r") as f:
            ...
            while line != None and line != "":
                ...
                line = f.readline()
        lists.append(items)
        print("already load the trainList...") #这里也需要加载
        return lists

    ##################### generate positive instances  生成正例，主要就是数组列表去分割数据
    ...
    def _preprocess(self,get_train_batch):  # generate the masked batch list  生成掩码批处理列表
