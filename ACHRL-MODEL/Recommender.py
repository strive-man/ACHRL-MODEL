...
class RecommenderNetwork(object):
    ...
    def __init__(self, sess, num_items, args):
        # 算是三个参数，不过 args参数是写在setting里的好几个参数
        ...
        self.target_user_input, self.target_num_idx, self.target_item_input, self.target_output, self.target_Q_, self.target_Q, self.target_W = self.create_recommender_network(
            "Target")
        self.target_network_params = tf.compat.v1.trainable_variables()[
                                     len(self.network_params) + self.num_other_variables:]
        ...
        self.labels = tf.compat.v1.placeholder(tf.int32, shape=[None, 1])  # the ground truth

        self.labels_reduce_dim = tf.reduce_sum(input_tensor=self.labels, axis=1)
        self.one_hot = tf.one_hot(self.labels_reduce_dim, self.classes)

        self.reward = tf.math.log(
            tf.reduce_sum(input_tensor=(self.reward_output_concat * self.one_hot + 1e-15), axis=1))
        self.l2loss = 0
        self.l2loss = self.lambda_bilinear * tf.reduce_sum(input_tensor=tf.square(self.target_Q)) + \
                      self.gamma_bilinear * tf.reduce_sum(input_tensor=tf.square(self.target_Q_)) + \
                      self.eta_bilinear * tf.reduce_sum(input_tensor=tf.square(self.target_W))

        self.loss = tf.compat.v1.losses.log_loss(self.labels, self.target_output)
        self.loss += self.l2loss
        self.gradients = tf.gradients(ys=self.loss, xs=self.target_network_params)
    ...
    def _create_variables(self, scope):
        with tf.compat.v1.name_scope(scope):  # The embedding initialization is unknown now
            self.c1 = tf.Variable(
            ...
            if self.algorithm == 0:
                self.W = tf.Variable(tf.random.truncated_normal(shape=[self.embedding_size, self.weight_size], mean=0.0,
                                                                stddev=tf.sqrt(
                                                                    tf.compat.v1.div(2.0,
                                                                                     self.weight_size + self.embedding_size))),
                                     name='Weights_for_MLP', dtype=tf.float32, trainable=True)
            else:
                self.W = tf.Variable(
                    tf.random.truncated_normal(shape=[2 * self.embedding_size, self.weight_size], mean=0.0,
                                               stddev=tf.sqrt(tf.compat.v1.div(2.0, self.weight_size + (
                                                       2 * self.embedding_size)))), name='Weights_for_MLP',
                    dtype=tf.float32, trainable=True)
            self.b = tf.Variable(tf.random.truncated_normal(shape=[1, self.weight_size], mean=0.0, stddev=tf.sqrt(
                tf.compat.v1.div(2.0, self.weight_size + self.embedding_size))), name='Bias_for_MLP', dtype=tf.float32,
                                 trainable=True)
    ...
    def _attention_MLP(self, q_, scope):
        with tf.compat.v1.name_scope(scope):
            ...
            exp_A_ = tf.exp(A_)
            num_idx = tf.reduce_sum(input_tensor=self.num_idx, axis=1)
            ...
            self.A = tf.expand_dims(tf.compat.v1.div(exp_A_, exp_sum), 2)  # (b, n, 1)

            return tf.reduce_sum(input_tensor=self.A * self.embedding_q_, axis=1)
    ...
    def _create_inference(self, scope):
        with tf.compat.v1.name_scope(scope):
            ...
            self.embedding_q = tf.reduce_sum(input_tensor=self.embedding_q, axis=1)
            ...
            self.output = tf.sigmoid(
                self.coeff * tf.expand_dims(tf.reduce_sum(input_tensor=self.embedding_p * self.embedding_q, axis=1),
                                            1) + self.bias_i)
